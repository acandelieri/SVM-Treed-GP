...
plot2D.labels <- function( svmtgp, lower, upper, n.grid = 30 ) {
  ...
  image2D( x=seq(lower[1],upper[1],length.out=n.grid), y=seq(lower[2],upper[2],length.out=n.grid), z=leaves, colkey=F, xlab="x1", ylab="x2", col=cm.colors(100), cex.axis=1.3, cex.lab=1.3 )
}